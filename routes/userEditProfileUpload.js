...
Router.post("/adminuploadedits", upload.array("file"), async (req, res) => {
  ...
  const uploader = async (path) =>
    await cloudinary.uploader.upload(path, {
      public_id: `image/${photographerId}/edits/${uuidv4()}iffpre}`,
      tags: `image`,
    });
  ...
  if (isSubmitedbefore) {
    isSubmitedbefore.Edited = true;
    isSubmitedbefore.Edited_Photos = [
      ...isSubmitedbefore.Edited_Photos,
      ...urls,
    ];
    await isSubmitedbefore.save();
    return res.status(200).json({
      message: " the selected images has been sent to client",
    });
  } else {
    return res.status(501).json({
      message: "this image has not been submited for edit request",
    });
  }
